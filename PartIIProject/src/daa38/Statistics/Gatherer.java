...
public class Gatherer {
	...
	public static final double HICCUPS_THRESHOLD = 10f;
	public static final int HICCUPS_SAMPLES = 2;
	public static final int RANDOM_SAMPLES = 50;
	...
	public static void gather(Integer pOrder, Integer pSelect, Integer pBack, ArrayList<String> pCSPFilesIn, ArrayList<String> pCSPFilesOut) throws IOException
	{
		...
		for (int lIt = 0; lIt < pCSPFilesIn.size(); lIt++)
		{
			...
			if (pOrder!=0)
			{
				...
				{
					System.out.println("Repeating. With "+pOrder+pSelect+pBack+". At file "+lFileIn);
					lPrevTime=lTime;
					lTime = lSolver.solve(lFileIn, lFileOut, lVO, lVS, lLB);
				}
			}
			else
			{
				double lAverage = lPrevTime+lTime;
				
				int lSamplesLeft = RANDOM_SAMPLES-2;
				
				while (lSamplesLeft>0)
				{
					lAverage += lSolver.solve(lFileIn, lFileOut, lVO, lVS, lLB);
					lSamplesLeft--;
				}
	...
	public static void main(String[] args) throws IOException {
		...
		*/
		...
		for (int lInstance = 1; lInstance <= 35; lInstance++)
		{
