...
public class NQueens {
	...
	{
		int[][] lMatrix = FileHandler.readNQueensSolution(pPath);
		return nQueensCheckMatrix(lMatrix);
	}
	...
	public static void main(String[] args) throws IOException {
		...
		int NUMBER_OF_QUEENS = 17;
		
		String lCSPProblemPath = "nQueensCSP_problem.txt";
		generateCSP(NUMBER_OF_QUEENS,lVars,lCons);
		FileHandler.writeFileProblem(lCSPProblemPath, lVars, lCons);
		
		String lCSPSolutionPath = "nQueensCSP_solution.txt";
		...
		FileHandler.readFileAssignment(lCSPSolutionPath, lVarsRead);
		
		int[][] lMatrix = CSPToNQueens(lVarsRead);
		String lNQueensSolutionPath = "nQueens_solution.txt";
		FileHandler.writeNQueensSolution(lNQueensSolutionPath, lMatrix);
		
		System.out.println(nQueensCheckFile(lNQueensSolutionPath));
	}
